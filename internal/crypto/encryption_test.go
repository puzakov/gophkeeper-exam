...
func TestEncryptDecryptSecret(t *testing.T) {
	...
	ciphertext, err := EncryptSecret(plaintext, dek)
	if err != nil {
		t.Fatal(err)
	}

	decrypted, err := DecryptSecret(ciphertext, dek)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDecryptSecret_WrongKey(t *testing.T) {
	...
	ciphertext, _ := EncryptSecret([]byte("data"), dek1)
	_, err := DecryptSecret(ciphertext, dek2)
	if err == nil {
...
func TestDecryptSecret_Tampered(t *testing.T) {
	...
	ciphertext, _ := EncryptSecret([]byte("data"), dek)
	ciphertext[len(ciphertext)/2] ^= 0xff

	_, err := DecryptSecret(ciphertext, dek)
	if err == nil {
